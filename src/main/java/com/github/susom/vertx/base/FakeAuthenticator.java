...
/**
 * This class provides fake authentication services based on an internal
 * (embedded) OpenID Connect provider. That provider simply presents a page
 * that allows you to choose your own user name, display name, authority, etc.
 *
 * @author garricko
 */
public class FakeAuthenticator implements Security {
  ...
  public FakeAuthenticator(Vertx vertx, Router root, SecureRandom secureRandom, Function<String, String> cfg) throws Exception {
    this.vertx = vertx;
    this.root = root;
...
//    });
//    Handler<RoutingContext> authenticateRequiredOrRedirect302 = rc -> {
//      if (rc.user() == null) {
//        cookieHandler.handle(rc);
//
//        // This will call rc.next()
    ...
    public void handle(RoutingContext rc) {
      ...
      if (user != null) {
        ...
        if (sessionCookie != null && sessionCookie.getValue() != null) {
          if (sessionCookie.getPath() != null && !"/".equals(sessionCookie.getPath())) {
            log.debug("Found a session cookie with incorrect path {}", sessionCookie.getPath());
            String path = "";
            for (String pathComponent : rc.request().path().split("/")) {
              if (!pathComponent.isEmpty()) {
                path += pathComponent;
